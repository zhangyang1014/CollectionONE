...
class TeamAdminAccount(Base):
    ...
    agency_id = Column(Integer, ForeignKey("collection_agencies.id"), nullable=False, index=True, comment="所属机构ID")
    team_id = Column(Integer, ForeignKey("collection_teams.id"), nullable=False, index=True, comment="所属小组ID")
    ...
    role = Column(String(50), nullable=False, index=True, comment="角色：team_leader/quality_inspector/statistician")
