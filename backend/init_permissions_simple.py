...
def init_permissions():
    ...
    try:
        ...
        existing_modules = cursor.fetchone()[0]
        ...
        for module_key, module_data in PERMISSION_DATA.items():
            # 创建模块
            cursor.execute("""
                INSERT INTO permission_modules (module_key, module_name, sort_order, is_active, created_at, updated_at)
                VALUES (?, ?, ?, 1, ?, ?)
            """, (module_key, module_data["module_name"], module_data["sort_order"], now, now))
            
            module_id = cursor.lastrowid
            print(f"  ✓ 创建模块: {module_data['module_name']} ({module_key})")
            
            # 创建该模块下的权限项
            for item_data in module_data["items"]:
                cursor.execute("""
                    INSERT INTO permission_items (module_id, item_key, item_name, description, sort_order, is_active, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, 1, ?, ?)
                """, (module_id, item_data["key"], item_data["name"], item_data["description"], item_data["sort"], now, now))
                
                item_id = cursor.lastrowid
                item_id_map[item_data["key"]] = item_id
                print(f"    ✓ 创建权限项: {item_data['name']} ({item_data['key']})")
        ...
        for item_key, role_permissions in DEFAULT_ROLE_PERMISSIONS.items():
            ...
            for role_code, permission_level in role_permissions.items():
                cursor.execute("""
                    INSERT INTO role_permission_configs (tenant_id, role_code, permission_item_id, permission_level, created_at, updated_at)
                    VALUES (NULL, ?, ?, ?, ?, ?)
                """, (role_code, item_id, permission_level, now, now))
                config_count += 1
