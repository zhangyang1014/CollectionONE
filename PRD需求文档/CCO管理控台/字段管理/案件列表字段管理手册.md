...
| 版本号 | 日期 | 作者 | 变更说明 |
|--------|------|------|---------|
| v1.0.0 | 2025-12-08 | 产品大象 | 初始版本，包含完整字段管理流程说明 |
...
- **路由**: `/field-config/tenant-fields-list`
...
优先级2: 映射配置最新版本（tenant_field_uploads表）
    ↓ 如果不存在
优先级3: 内置Mock字段（15个基础字段）
```

### 5.5 Mock字段列表

系统内置15个基础字段作为兜底数据（与标准字段保持一致）：

| 序号 | 字段名称 | 字段标识 | 类型 | 必填 |
|------|---------|---------|------|------|
| 1 | 案件编号 | case_code | String | ✓ |
| 2 | 客户姓名 | user_name | String | ✓ |
| 3 | 手机号 | mobile_number | String | ✓ |
| 4 | 贷款金额 | loan_amount | Decimal | ✓ |
| 5 | 未还金额 | outstanding_amount | Decimal | ✓ |
| 6 | 逾期天数 | overdue_days | Integer | ✓ |
| 7 | 案件状态 | case_status | Enum | ✓ |
| 8 | 到期日期 | due_date | Date | ✓ |
| 9 | 期数 | total_installments | Integer | ✓ |
| 10 | 当期天数 | term_days | Integer | ✓ |
| 11 | 所属系统 | system_name | String | ✓ |
| 12 | 产品名称 | product_name | String | ✓ |
| 13 | APP名称 | app_name | String | ✓ |
| 14 | 商户名称 | merchant_name | String | ✓ |
| 15 | 关联借款ID | loan_id | String | ✓ |
...
GET /api/v1/tenant-field-configs/mapping?tenantId=1&scene=list
...
#### 6.3.2 sceneType参数（用于展示配置）

**参数说明**：
- **参数名**: `sceneType`
- **可选值**: `admin_case_detail`（控台案件列表）、`collector_case_detail`（IM端案件列表）
- **使用场景**: 字段展示配置、前端页面渲染
...
POST /api/v1/case-list-field-configs/version/save
  Body: { "sceneType": "admin_case_detail", ... }
```
...
| **API路径** | `/tenant-field-uploads`<br>`/tenant-field-configs` | `/case-list-field-configs` |
| **版本管理** | 数据库表（`tenant_field_uploads`）| 本地JSON文件 |
| **命名历史** | 清晰明确 | 保留历史`_detail`后缀（实际用于列表） |
...
scene层级（甲方字段维度）
    ├── list（列表场景）
    │   └── sceneType层级（展示配置维度）
    │       ├── admin_case_detail（控台列表）
    │       └── collector_case_detail（IM端列表）
    └── detail（详情场景）
        └── sceneType层级（展示配置维度）
            ├── admin_case_detail（控台详情）
            └── collector_case_detail（IM端详情）
```

**实际使用逻辑**：
1. 甲方上传字段时指定 `scene=list`
2. 映射配置基于 `scene=list` 的甲方字段
3. 控台端展示配置使用 `sceneType=admin_case_detail`，从 `scene=list` 的映射配置中读取字段
4. IM端展示配置使用 `sceneType=collector_case_detail`，同样从 `scene=list` 的映射配置中读取字段
...
系统中存在**多个字段名**表示枚举值，为了兼容性考虑，系统会自动识别：

#### 9.8.1 字段名对照表

| 字段名 | 使用场景 | 说明 |
|--------|---------|------|
| `enum_values` | 甲方JSON、后端数据库 | 甲方上传时使用的字段名 |
| `enum_options` | 前端展示、API响应 | 前端组件使用的字段名 |
| `enumValues` | 部分Java代码 | 驼峰命名风格 |

#### 9.8.2 兼容性处理

**后端处理逻辑**：
```
// CaseListFieldConfigController.java
...
**含义**：
- 后端会优先查找 `enum_options`
- 如果不存在，则查找 `enum_values`
- 确保两种命名方式都能正常工作
...
interface FieldConfig {
  enumOptions?: string[];  // 枚举值数组
}
```

#### 9.8.3 使用建议

**甲方JSON上传**（推荐使用 `enum_values`）：
```
{
  "field_key": "case_status",
  "field_type": "Enum",
  "enum_values": ["待分配", "催收中", "已完成"]
}
```

**后端Mock数据**（推荐使用 `enum_values`）：
```
templateField.put("enum_values", Arrays.asList("待分配", "催收中", "已完成"));
```

**前端展示**（统一使用 `enumOptions`）：
```
...
⚠️ **重要提醒**：
- `enum_values` 和 `enum_options` 本质上是**同一个数据**，只是命名不同
- 系统会自动转换，无需手动处理
- 新代码推荐统一使用 `enum_values`（后端）和 `enumOptions`（前端）
- 避免在同一个对象中同时使用两个字段名，防止混淆
