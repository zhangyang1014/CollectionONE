...
| 层级 | 名称 | 作用 | 数据来源 | 核心特点 |
|------|------|------|----------|----------|
| **Layer 0** | 分组管理 | 管理分组基础数据（CRUD） | 系统内置+自定义（`field_groups_detail`表） | 5个标准分组+自定义分组 |
| **Layer 1** | 标准字段 | 系统预定义的字段规范（含分组） | 系统内置（`standard_fields_detail`表） | 5个分组，21个字段 |
...
系统预定义5个标准分组，不可删除，group_key不可修改：

| 分组标识 | 分组名称 | 排序 | 用途 |
|---------|---------|------|------|
| basic_info | 基本信息 | 1 | 案件基础信息 |
| loan_info | 借款信息 | 2 | 借款相关信息 |
| collection_info | 催收信息 | 3 | 催收相关信息 |
| contact_info | 联系信息 | 4 | 联系方式信息 |
| product_info | 产品信息 | 5 | 产品相关信息 |
