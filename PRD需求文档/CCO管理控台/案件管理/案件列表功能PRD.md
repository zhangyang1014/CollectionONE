...
- **必须展示字段**（固定展示，不可配置隐藏）：
  ...
  - `case_status`：案件状态
  - `due_date`：到期日期
- **可配置字段**：除必须展示字段外的其他字段可以通过配置选择是否展示
- **特殊字段规则**：
...
| due_date_start | String | 否 | 到期日期开始（YYYY-MM-DD） | 第三排 |
| due_date_end | String | 否 | 到期日期结束（YYYY-MM-DD） | 第三排 |
| overdue_days_min | Integer | 否 | 逾期天数最小值（新增） | 第三排 |
| overdue_days_max | Integer | 否 | 逾期天数最大值（新增） | 第三排 |
