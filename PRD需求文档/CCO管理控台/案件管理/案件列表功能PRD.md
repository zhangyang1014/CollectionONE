...
- 客户姓名（模糊匹配）
- 客户ID（精确匹配）
- 手机号码（精确匹配或模糊匹配）- **注意**：手机号码默认脱敏展示且可点击展开，同时支持搜索
...
| installment_no | Integer/String | 期数 | 固定筛选器 | 第三排（第1行） |
| first_term_days | Integer/String | 首期天数 | 固定筛选器 | 第三排（第1行） |
| system_name | String | 所属系统（iOS/Android/Web） | 固定筛选器 | 第三排（第1行） |
| merchant_name | String | 商户 | 固定筛选器（下拉，选项来自已加载案件） | 第三排（第1行） |
| app_name | String | APP | 固定筛选器（下拉，选项来自已加载案件） | 第三排（第2行） |
| product_name | String | 产品 | 固定筛选器（下拉，选项来自已加载案件） | 第三排（第2行） |
...
**筛选器区域**（多排紧凑布局：前两排固定 + 第三排固定新增筛选器两行 + 第四排起动态筛选器）：
...
**第四排起**（动态筛选器）：
- 来源：甲方字段展示配置中 `is_filterable = true` 的枚举字段
- 布局：每行4个，超过自动换行；顺序按字段展示配置 `sort_order`
- 交互：筛选条件变更后自动触发查询；支持单选/多选（按字段配置）
