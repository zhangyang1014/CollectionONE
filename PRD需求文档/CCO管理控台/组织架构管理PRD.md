...
1. **创建甲方**（SuperAdmin操作）
   - 输入甲方基本信息：名称、编码、国家、时区、货币
   - 输入甲方管理员信息：账号名、登录ID、邮箱、密码
   - 系统创建甲方记录和甲方管理员账号
   - 甲方管理员收到账号信息

2. **创建机构**（甲方管理员操作）
   - 选择所属甲方（通常为当前登录用户的甲方）
   - 输入机构信息：机构编码、机构名称、联系方式
   - 选择或创建机构管理员账号
   - 系统创建机构记录和机构管理员账号
   - 机构管理员收到账号信息
...
- **删除限制**：如果组织单元下有关联的小组、催员或案件，不允许删除
- **删除顺序**：必须先删除或转移所有下级组织单元，才能删除上级组织单元
...
- **敏感信息加密**：催员的密码必须加密存储，使用安全的哈希算法（如BCrypt）
...
| country | String(50) | 国家 | 用户选择 | 可编辑 |
| timezone | String(50) | 时区 | 用户选择 | 可编辑 |
| currency | String(10) | 货币 | 用户选择 | 可编辑 |
| is_active | Boolean | 是否启用 | 系统设置 | 可编辑 |
| created_at | DateTime | 创建时间 | 系统生成 | 自动 |
| updated_at | DateTime | 更新时间 | 系统生成 | 自动 |

#### 7.2 机构（CollectionAgency）核心字段
...
| admin_id | BigInteger | 机构管理员ID | 关联选择 | 可编辑 |
...
| spv_id | BigInteger | 小组群长SPV ID | 关联选择 | 可编辑 |
...
| username | String(100) | 登录账号（唯一） | 用户输入 | 创建时设置，不可修改 |
| name | String(200) | 管理员姓名 | 用户输入 | 可编辑 |
| email | String(100) | 邮箱 | 用户输入 | 可编辑 |
| password_hash | String(255) | 密码哈希 | 系统生成 | 可编辑（重置密码） |
| is_active | Boolean | 是否启用 | 系统设置 | 可编辑 |
| created_at | DateTime | 创建时间 | 系统生成 | 自动 |
| updated_at | DateTime | 更新时间 | 系统生成 | 自动 |
| last_login_at | DateTime | 最近登录时间 | 系统更新 | 登录时更新 |

#### 7.7 催员（Collector）核心字段
...
| username | String(100) | 登录账号（唯一） | 用户输入 | 创建时设置，不可修改 |
| password_hash | String(255) | 密码哈希 | 系统生成 | 可编辑（重置密码） |
| role | String(50) | 角色（collector/leader/spv） | 用户选择 | 可编辑 |
| mobile | String(50) | 手机号 | 用户输入 | 可编辑 |
| email | String(100) | 邮箱 | 用户输入 | 可编辑 |
| is_active | Boolean | 是否启用 | 系统设置 | 可编辑 |
| created_at | DateTime | 创建时间 | 系统生成 | 自动 |
| updated_at | DateTime | 更新时间 | 系统生成 | 自动 |
...
- **列表展示**：机构编码、名称、所属甲方、管理员、小组数量、催员数量、状态、操作
- **筛选功能**：按甲方筛选
...
- **列表展示**：小组群编码、名称、所属机构、SPV、小组数量、催员数量、状态、操作
- **筛选功能**：按甲方、机构筛选
- **操作按钮**：创建小组群、编辑、启用/禁用
...
- **列表展示**：账号、姓名、所属小组、邮箱、最近登录时间、状态、操作
- **筛选功能**：按甲方、机构、小组筛选（三级联动）
- **操作按钮**：创建小组管理员、编辑、重置密码、启用/禁用
...
| POST | `/` | 创建甲方 | tenant_code、tenant_name、country、timezone、currency、admin_info | 创建的甲方信息 | 5s/不重试 | 是（基于tenant_code） |
...
| POST | `/` | 创建机构 | tenant_id、agency_code、agency_name、admin_info | 创建的机构信息 | 5s/不重试 | 是（基于agency_code） |
...
| POST | `/` | 创建小组群 | tenant_id、agency_id、group_code、group_name、spv_id（可选） | 创建的小组群信息 | 5s/不重试 | 是（基于group_code） |
...
| spv_id | BIGINT | NULL, FOREIGN KEY | 小组群长SPV ID |
