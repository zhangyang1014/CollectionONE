...
| 版本 | 日期 | 变更内容 | 变更人（大象） |
| --- | --- | --- | --- |
| 1.1.0 | 2025-12-10 | 新增组织架构树状图展示功能，支持树形结构浏览和从树节点创建催员 | 大象 |
| 1.0.1 | 2025-12-09 | 登录人脸查询改为跳转"人脸比对"并默认筛选当前催员 | 大象 |
| 1.0.0 | 2025-11-22 | 初版 | 大象 |
...
- 按钮文字："登录人脸查询"
- 按钮样式：链接按钮（信息色）
- 点击后：跳转至"AI质检-人脸比对"页面（菜单项"人脸比对"高亮）
- 跳转参数：携带当前催员所属机构ID、小组ID、小组群ID（如有）、催员ID
- 页面默认动作：人脸比对页自动回填筛选项并立即查询，仅展示当前催员的登录人脸比对记录
- 操作后可在目标页继续切换其他催员或条件
...
在催员管理页面采用左右分栏布局，左侧以树状图形式展示组织架构层级（机构→小组群→小组→催员），右侧保留原有的催员列表表格和筛选功能。通过树状图可以快速浏览组织架构、定位催员位置，并支持从树节点直接创建催员，提升管理效率。
...
| 节点类型 | 图标 | 显示内容 | 额外信息 |
|----------|------|----------|----------|
| 机构节点 | 🏢 OfficeBuilding | 机构名称 | - |
| 小组群节点 | 📊 Grid | 小组群名称 | - |
| 小组节点 | 👥 User | 小组名称 | 催员数量标签（如"5人"） |
| 催员节点 | 👤 Avatar | 催员姓名 | - |
...
|------|----------|------|
| 标题 | "组织架构" | 固定显示在左上角 |
| 诊断按钮 | "诊断" | 点击后在控制台输出树数据统计信息，用于调试 |
| 刷新按钮 | "刷新" | 重新加载树状图数据，显示loading状态 |

**树节点交互：**

1. **展开/收起**
   - 默认：所有节点全部展开（`default-expand-all`）
   - 点击节点前的展开图标可展开/收起子节点
   - 点击节点内容区域不会触发展开/收起
...
2. **节点点击筛选**
   ...
   | 点击节点类型 | 筛选行为 | 右侧表格显示 |
   |-------------|----------|--------------|
   | 机构节点 | 设置机构筛选器为该机构，清空小组筛选 | 该机构的所有催员 |
   | 小组群节点 | 设置机构筛选器，清空小组筛选 | 该机构的所有催员（待优化：只显示该小组群下的催员） |
   | 小组节点 | 设置机构和小组筛选器 | 该小组的所有催员 |
   | 催员节点 | 在搜索框填充催员名称或登录ID | 高亮显示该催员 |

3. **小组节点的"创建催员"按钮**
   
   - **触发方式**：hover到小组节点时，节点右侧显示"创建催员"文字按钮
   - **按钮样式**：文字按钮，主色调（蓝色）
   - **点击效果**：
     - 打开催员配置表单对话框
     - 对话框标题：`为小组"{小组名称}"创建催员`
     - 自动填充"所属机构"和"所属小组"字段
     - 禁用"所属机构"和"所属小组"的下拉选择器
     - 在禁用的选择器下方显示灰色提示："从树状图创建，机构已自动选择" / "从树状图创建，小组已自动选择"
   - **保存后行为**：
     - 创建成功后刷新树状图和右侧表格
     - 在树状图中找到该小组节点并展开
     - 新创建的催员出现在小组子节点列表中
...
2. **筛选器 → 树（单向）**
   - 改变右侧机构或小组筛选器时，树自动高亮对应节点
   - 自动展开父节点以确保选中节点可见
   - 支持的筛选器：机构筛选、小组筛选

**空数据提示：**

当树状图无数据时（`treeData.length === 0`），显示信息提示框：
...
- 组件：Element Plus 的 `el-tree` 组件
- 布局：`el-container` + `el-aside` + `el-main`
- 数据加载：分层异步加载（机构→小组群→小组→催员）
- 节点标识：使用前缀+ID的方式（如：`agency-1`, `team-group-2`, `team-3`, `collector-4`）

**验收标准：**

- [ ] 页面打开后自动加载树状图，展示所有层级
- [ ] 树节点图标正确显示（机构/小组群/小组/催员）
- [ ] 小组节点显示催员数量标签
- [ ] hover小组节点显示"创建催员"按钮
- [ ] 点击"创建催员"按钮打开对话框，机构和小组已自动填充且禁用
- [ ] 从树创建催员成功后，树和表格都自动刷新
- [ ] 点击树节点能正确筛选右侧表格
- [ ] 改变筛选器能自动高亮树节点
- [ ] "刷新"按钮能重新加载树数据
...
**跳转与筛选：**
- 跳转目标：点击后路由至"AI质检-人脸比对"页面（/ai-quality/face-compare），左侧菜单自动高亮"人脸比对"
- 携带参数：collectorId、agencyId、teamGroupId（如有）、teamId，用于精准锁定当前催员
- 自动回填：目标页筛选表单自动回填上述参数，并立即触发查询
- 查询结果：默认只展示当前催员的登录人脸比对记录，可在目标页继续调整筛选条件

**记录展示（AI质检-人脸比对页沿用现有样式）：**
- 表格列：催员ID、所属机构/小组群/小组、比对结果、首次/当日/昨日/前日照片、是否禁用
- 照片列：展示缩略图，支持点击预览；加载失败时显示占位符
- 分页：支持每页10/20/50/100切换
- 空状态：无记录时展示空列表提示
...
**跳转与预筛选：**
- 点击操作列"登录人脸查询"后跳转至"AI质检-人脸比对"页面，左侧菜单高亮该项
- 路由参数：agencyId、teamGroupId（如有）、teamId、collectorId，用于锁定当前催员
- 目标页自动回填筛选项并立即触发查询，默认只展示该催员的登录人脸比对记录
- 若参数缺失则保持为空，用户可继续选择其他筛选项

**展示交互（沿用人脸比对页）：**
- 结果以表格呈现，展示比对结果与多日照片缩略图，支持点击预览
- 结果支持分页、锁定状态标签显示
- 空状态时展示空列表提示
...
- 人脸记录查询：点击"登录人脸查询"跳转至人脸比对页并自动过滤至当前催员，结果加载正确
